...
class Segment:
    ...
    def __init__(self, forward):
        ...
        self.dwf = np.array([]) 
        self.dvf = np.array([]) 
        self.weight = np.ones(forward.u.shape[1])
        self.weight[0] = self.weight[-1] = 0.5
    ...
    def run1seg(self, interface, forward, stepfunc):
        ...
        C = self.get_C_cts(w)
        dwv = self.get_d_cts_wp(w, vst)
        dwf = self.get_d_cts_wp(w, f)
        dvf = self.get_d_cts_vf(vst, f)
    ...
    def get_C_cts(self, w):
        # compute the covariant matrix using w on all time steps
        # w is on a segment, shape (nstep_per_segment+1, M, m)
        assert w.ndim == 3
        ...
        for i in range(M_modes):
            for j in range(M_modes):
                C[i,j] = np.sum( w[:, i, :] * w[:, j, :] * self.weight[:, np.newaxis] )
                C[j,i] = C[i,j]
        return C


    def get_d_cts_wp(self, w, p):
        # p is either vstar or f, shape (nstep_per_segment+1, m)
        assert p.ndim == 2
        assert w.shape[2] == p.shape[1]
        M_modes = w.shape[1]
        d = np.zeros(M_modes)
        for i in range(M_modes):
            d[i] = np.sum( w[:, i, :] * p * self.weight[:, np.newaxis])
        return d


    def get_d_cts_vf(self, v, f):
        # vstar and f, shape (nstep_per_segment+1, m)
        assert v.shape == f.shape
        return np.sum(v*f * self.weight[:,np.newaxis])
